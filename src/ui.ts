...
function renderPurchaseDetail(view: AppView): string {
  ...
  const remainingBalance = getRemainingBalance(purchase)
  const tenYearValue = getOpportunityValue(purchase, 10)
  const potentialGrowth = Math.max(0, tenYearValue - remainingBalance)
  const trueCostSummary = `<section class="truecost-summary"><p class="eyebrow">TrueCost summary</p><h2>Your remaining ${formatMoney(remainingBalance, view.settings.currency)} in payments could be worth ${formatMoney(tenYearValue, view.settings.currency)} in 10 years if invested at 8%.</h2><p>That is an estimated ${formatMoney(potentialGrowth, view.settings.currency)} in potential growth. This is an educational illustration, not a guaranteed investment return.</p></section>`
  return `<section class="detail-page"><button class="back-button" data-page="purchases">Back to purchases</button><div class="page-heading"><div><p class="eyebrow">Purchase details</p><h1>${escapeHtml(purchase.name)}</h1></div><div class="card-actions"><button data-action="edit" data-id="${purchase.id}">Edit purchase</button><button class="primary-button" data-action="manage-payments" data-id="${purchase.id}">Manage payments</button></div></div><section class="summary-grid detail-summary">${summaryCard('Original price', formatMoney(purchase.totalPrice, view.settings.currency))}${summaryCard('Remaining balance', formatMoney(remainingBalance, view.settings.currency))}${summaryCard('Payment interval', `${purchase.paymentIntervalValue} ${purchase.paymentIntervalUnit}`)}</section>${trueCostSummary}<section class="panel"><h2>Potential value of these payments</h2><p>Assumes an 8% annual return, using the S&P 500 as an educational benchmark.</p><div class="summary-grid detail-summary">${opportunityValues}</div></section><section class="panel"><h2>Next 3 payment dates</h2><ol class="payment-list">${paymentRows}</ol></section>${renderPaymentTimeline(purchase, view.settings)}</section>`
}
